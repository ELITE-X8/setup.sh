...
#  + Bandwidth GB Limits + Auto User Info + Banner Toggle
# ╚══════════════════════════════════════════════════════════════╝
set -euo pipefail
...
DISPLAY_MODE_FILE="/etc/elite-x/display_mode"
...
show_banner() {
    ...
    echo -e "${PURPLE}║${CYAN}${BOLD}     TURBO BOOST EDITION - FULL OPTIMIZED + USER INFO TOGGLE   ${PURPLE}║${NC}"
    echo -e "${PURPLE}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
# ═══════════════════════════════════════════════════════════════
# SSH DISPLAY SETUP - Using Banner and update-motd hybrid
# ═══════════════════════════════════════════════════════════════
setup_ssh_display() {
    echo -e "${YELLOW}🔧 Setting up SSH Dynamic Display System...${NC}"
    
    # Create the display script that runs on login
    cat > /usr/local/bin/elite-x-ssh-display <<'DISPLAYEOF'
#!/bin/bash
# ELITE-X Dynamic SSH Display Script
# This script is called from /etc/profile.d and .bashrc for reliable execution

USER_DB="/etc/elite-x/users"
BANDWIDTH_DIR="/etc/elite-x/bandwidth"
DISPLAY_MODE_FILE="/etc/elite-x/display_mode"
BANNER_DIR="/etc/elite-x/banner"
CUSTOM_BANNER="$BANNER_DIR/ssh-banner"
DEFAULT_BANNER="$BANNER_DIR/default"
...
get_connection_count() {
    ...
    [ "$count" -eq 0 ] && count=$(ps aux | grep "sshd:" | grep "@" | grep "$username" | grep -v grep | wc -l)
    echo ${count:-0}
}
...
show_user_info() {
    ...
    if [ ! -f "$USER_DB/$username" ]; then
        # User not in our system, show banner
        if [ -f "$CUSTOM_BANNER" ]; then
            cat "$CUSTOM_BANNER"
        elif [ -f "$DEFAULT_BANNER" ]; then
            cat "$DEFAULT_BANNER"
        fi
        return
    fi
    ...
    if passwd -S "$username" 2>/dev/null | grep -q "L"; then
        lock_status="${RED}⚠️  ACCOUNT LOCKED  ⚠️${NC}"
    fi
    ...
    if [ -n "$lock_status" ]; then
        echo -e "${PURPLE}║${NC}          ${lock_status}"
        echo -e "${PURPLE}║${NC}"
    fi
    echo -e "${PURPLE}║${WHITE}  👤 File Name    : ${LIGHT_GREEN}${BOLD}${username}${NC}"
    echo -e "${PURPLE}║${WHITE}  🔑 Password     : ${YELLOW}${password:-N/A}${NC}"
    echo -e "${PURPLE}║${NC}"
...
# Main logic
USERNAME="${1:-$(whoami 2>/dev/null || echo 'root')}"
...
# Read display mode
DISPLAY_MODE=$(cat "$DISPLAY_MODE_FILE" 2>/dev/null || echo "banner")

if [ "$DISPLAY_MODE" = "userinfo" ]; then
    show_user_info "$USERNAME"
else
    show_banner_display
fi
DISPLAYEOF
    chmod +x /usr/local/bin/elite-x-ssh-display

    # Create profile.d script (runs for all users on login)
    cat > /etc/profile.d/elite-x-display.sh <<'PROFILEEOF'
#!/bin/bash
# ELITE-X Display Profile Script
# This runs for ALL users when they login

# Prevent running multiple times
if [ -n "${ELITE_X_DISPLAY_SHOWN:-}" ]; then
    return 0
fi
export ELITE_X_DISPLAY_SHOWN=1

# Only run for SSH sessions
if [ -n "${SSH_CONNECTION:-}" ] || [ -n "${SSH_TTY:-}" ] || [ "$(pstree -s $$ 2>/dev/null | grep -c sshd)" -gt 0 ]; then
    # Get the actual username
    CURRENT_USER="${USER:-$(whoami 2>/dev/null)}"
    
    # Call the display script
    if [ -x /usr/local/bin/elite-x-ssh-display ]; then
        /usr/local/bin/elite-x-ssh-display "$CURRENT_USER"
    fi
fi
PROFILEEOF
    chmod +x /etc/profile.d/elite-x-display.sh

    # Also add to skel for new users
    mkdir -p /etc/skel
    cat >> /etc/skel/.profile <<'SKELPROF'
# ELITE-X Display
if [ -f /etc/profile.d/elite-x-display.sh ]; then
    . /etc/profile.d/elite-x-display.sh
fi
SKELPROF

    # Configure sshd to use Banner for root
    # First remove any existing Banner/ForceCommand lines
    sed -i '/^Banner/d' "$SSHD_CONFIG" 2>/dev/null || true
    sed -i '/^ForceCommand.*elite-x/d' "$SSHD_CONFIG" 2>/dev/null || true
    sed -i '/elite-x-ssh-display/d' "$SSHD_CONFIG" 2>/dev/null || true
    sed -i '/^Match.*Group.*root.*ForceCommand/d' "$SSHD_CONFIG" 2>/dev/null || true
    
    # Add Banner for root login
    echo "" >> "$SSHD_CONFIG"
    echo "# ELITE-X Banner for root SSH login" >> "$SSHD_CONFIG"
    echo "Banner /etc/elite-x/banner/ssh-banner" >> "$SSHD_CONFIG"
    
    # Enable PrintMotd and UsePAM for profile.d to work
    ...
    echo -e "${GREEN}✅ SSH Display System configured${NC}"
}
...
show_dashboard() {
    ...
    DISPLAY_MODE=$(cat "$DISPLAY_MODE_FILE" 2>/dev/null || echo "banner")
    [ "$DISPLAY_MODE" = "userinfo" ] && DISPLAY_STATUS="${GREEN}USER INFO${NC}" || DISPLAY_STATUS="${YELLOW}BANNER${NC}"
    ...
    echo -e "${PURPLE}║${WHITE}  Display   :${CYAN} $DISPLAY_STATUS${NC}"
...
settings_menu() {
    while true; do
        ...
        echo -e "${PURPLE}║${WHITE}  [9] Toggle Display [Currently: $DISPLAY_STATUS]${WHITE}${NC}"
        echo -e "${PURPLE}║${WHITE}  [10] Restart All  [11] Reboot VPS      [12] Uninstall${NC}"
        ...
        case $ch in
            ...
            4) nano /etc/elite-x/banner/ssh-banner; echo -e "${GREEN}✅ Banner updated${NC}"; read -p "Press Enter..." ;;
            5) cp /etc/elite-x/banner/default /etc/elite-x/banner/ssh-banner; echo -e "${GREEN}✅ Banner Reset to Default${NC}"; read -p "Press Enter..." ;;
            6)
            ...
            8)
                ...
                ;;
            9)
                CURRENT_DISPLAY=$(cat "$DISPLAY_MODE_FILE" 2>/dev/null || echo "banner")
                if [ "$CURRENT_DISPLAY" = "userinfo" ]; then                    echo "banner" > "$DISPLAY_MODE_FILE"
                    echo -e "${YELLOW}✅ Display Mode changed to: BANNER${NC}"
                    echo -e "${GRAY}Users will see the server banner on login${NC}"
                else
                    echo "userinfo" > "$DISPLAY_MODE_FILE"
                    echo -e "${GREEN}✅ Display Mode changed to: USER INFO${NC}"
                    echo -e "${GRAY}Users will see their account details on login${NC}"
                fi
                # No need to restart SSH, changes take effect on next login
                echo -e "${GREEN}✅ Changes will take effect on next login${NC}"
                read -p "Press Enter..."
                ;;
            10)
                ...
                read -p "Press Enter..."
                ;;
            11) read -p "Reboot VPS? (y/n): " c; [ "$c" = "y" ] && reboot ;;
            12)
                ...
                [ "$c" = "YES" ] && {
                    ...
                    rm -f /etc/profile.d/elite-x-display.sh
                    sed -i '/^Banner/d' /etc/ssh/sshd_config 2>/dev/null
                    sed -i '/elite-x/d' /etc/ssh/sshd_config 2>/dev/null
...
╔═══════════════════════════════════════════════════════════════╗
║              ELITE-X v3.3.1 FALCON ENHANCED (FULL)           ║
║     C Proxy • IPv6 Off • BBR • 20MB Buff • Nice -20          ║
║            Type: menu  |  elite-x  |  users                  ║
╚═══════════════════════════════════════════════════════════════╝
EOF
cp /etc/elite-x/banner/default /etc/elite-x/banner/ssh-banner

# Setup SSH display (Profile.d method - works reliably)
setup_ssh_display
...
# Setup auto-login dashboard (only for root/interactive)
cat > /etc/profile.d/elite-x-dashboard.sh <<'EOF'
#!/bin/bash
# ELITE-X Dashboard - Only show for root on interactive login
...
echo -e "${GREEN}║${WHITE}  Display    :${CYAN} Dynamic (Profile.d - Banner/User Info)${NC}"
...
echo ""
echo -e "${YELLOW}═════════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}📋 HOW DISPLAY TOGGLE WORKS:${NC}"
echo -e "${WHITE}  • Uses /etc/profile.d/ system - reliable for all SSH logins${NC}"
echo -e "${WHITE}  • Toggle in Settings Menu → Option [9]${NC}"
echo -e "${WHITE}  • Changes take effect on NEXT login${NC}"
echo -e "${YELLOW}═════════════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "${GREEN}Commands:${NC} menu | elite-x | users | adduser | setbw"
