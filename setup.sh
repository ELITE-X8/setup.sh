...
#  + GB Limits + C Boosters + Auto-Delete + VPN FIX + BANNER SYSTEM
# ╚══════════════════════════════════════════════════════════════╝
set -euo pipefail
...
USER_DETAILS_DIR="/etc/elite-x/user_details"

show_banner_boot() {
    clear
    echo -e "${PURPLE}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}║${YELLOW}${BOLD}      ELITE-X SLOWDNS v3.3.3 FALCON ULTIMATE EDITION        ${PURPLE}║${NC}"
    echo -e "${PURPLE}║${GREEN}${BOLD}  GB Limits • C Boosters • Auto-Delete • VPN • Banner System  ${PURPLE}║${NC}"
...
# ═══════════════════════════════════════════════════════════
# BANNER SYSTEM - ADVANCED
# ═══════════════════════════════════════════════════════════
setup_banner_system() {
    echo -e "${YELLOW}🎨 Setting up Advanced Banner System...${NC}"
    
    mkdir -p "$BANNER_DIR" "$SERVER_MSG_DIR" "$USER_DETAILS_DIR"
...
║  📊 Monitor your usage with: menu                            ║
║  🔧 Check bandwidth: elite-x-user details                     ║
╚═══════════════════════════════════════════════════════════════╝
EOF

    # Server message status flag
    echo "default" > "$SERVER_MSG_DIR/active"
    
    # Copy default banner
    cp "$BANNER_DIR/default" "$BANNER_DIR/ssh-banner"
    
    # Setup SSH to use banner
    ...
    # Create user details script for auto-display on login
    cat > /etc/profile.d/elite-x-user-details.sh <<'PROFILEEOF'
#!/bin/bash
# ELITE-X User Details Display on Login
...
if [ -n "$SSH_CONNECTION" ] && [ "$USER" != "root" ]; then
    ...
    # Get active server message type
    ...
    if [ -f "$USER_DB/$USER" ]; then
        ...
        [ "$current_conn" -eq 0 ] && current_conn=$(ps aux | grep "sshd:" | grep "$USER" | grep -v grep | grep -v "sshd:.*@notty" | wc -l)
        ...
        # Display user details in the requested format
        ...
        echo -e "\033[1;36m║\033[0m \033[1;37mExpire       :\033[0m \033[1;31m${expire_date}${NC} (\033[1;32m${remaining}\033[0m)"
    else
        echo -e "\033[1;36m║\033[0m \033[1;31m⚠️  Account details not found in database\033[0m"
    fi
...
PROFILEEOF
    ...
    echo -e "${GREEN}✅ Banner System setup complete${NC}"
}
...
manage_banners() {
    while true; do
        ...
        echo -e "${PURPLE}║${WHITE}  [4] Create New Server Message Template${NC}"
        echo -e "${PURPLE}║${WHITE}  [5] View Current SSH Banner${NC}"
        echo -e "${PURPLE}║${WHITE}  [6] View Active Server Message${NC}"
        echo -e "${PURPLE}║${WHITE}  [7] Reset Both to Default${NC}"
        echo -e "${PURPLE}║${WHITE}  [8] Delete Server Message Template${NC}"
        echo -e "${PURPLE}║${WHITE}  [0] Back to Settings${NC}"
        echo -e "${PURPLE}╠════════════════════════════════════════════════════════════════╣${NC}"
        echo -e "${PURPLE}║${CYAN}  Active Server Message: ${GREEN}$ACTIVE_MSG${NC}"
        echo -e "${PURPLE}╚════════════════════════════════════════════════════════════════╝${NC}"
        ...
        case $ch in
            1)
                clear
                echo -e "${YELLOW}Editing SSH Banner (shown BEFORE login)...${NC}"
                echo -e "${CYAN}Current banner:${NC}"
                cat "$BANNER_DIR/ssh-banner"
                echo ""
                read -p "Press Enter to edit with nano..."
            ...
            2)
                clear
                echo -e "${YELLOW}Editing Active Server Message (shown AFTER login)...${NC}"
            ...
            3)
                clear
                echo -e "${YELLOW}Available Server Messages:${NC}"
                ls -1 "$SERVER_MSG_DIR/"*.msg 2>/dev/null || echo "No custom .msg files"
                ls -1 "$SERVER_MSG_DIR/" | grep -v "active" | grep -v ".msg"
                echo ""
                read -p "$(echo -e $GREEN"Enter message name to activate: "$NC)" msg_name
                if [ -f "$SERVER_MSG_DIR/$msg_name" ]; then
                    echo "$msg_name" > "$SERVER_MSG_DIR/active"
                    echo -e "${GREEN}✅ Server message switched to: $msg_name${NC}"
                elif [ -f "$SERVER_MSG_DIR/${msg_name}.msg" ]; then
                    echo "${msg_name}.msg" > "$SERVER_MSG_DIR/active"
                    echo -e "${GREEN}✅ Server message switched to: ${msg_name}.msg${NC}"
                else
            ...
            4)
                clear
                read -p "$(echo -e $GREEN"New message name [without spaces]: "$NC)" new_name
                [ -z "$new_name" ] && { echo -e "${RED}Invalid name!${NC}"; read -p "Press Enter..."; continue; }
                
                cat > "$SERVER_MSG_DIR/${new_name}.msg" <<'TEMPEOF'
╔═══════════════════════════════════════════════════════════════╗
║              YOUR CUSTOM MESSAGE HERE                         ║
║              Edit this template as you wish                   ║
╚═══════════════════════════════════════════════════════════════╝
TEMPEOF
                nano "$SERVER_MSG_DIR/${new_name}.msg"
                echo -e "${GREEN}✅ New message template created: ${new_name}.msg${NC}"
                read -p "$(echo -e $GREEN"Activate this message now? (y/n): "$NC)" act
                [ "$act" = "y" ] && { echo "${new_name}.msg" > "$SERVER_MSG_DIR/active"; echo -e "${GREEN}✅ Activated!${NC}"; }
                read -p "Press Enter..."
                ;;
            5)
                clear
                echo -e "${CYAN}Current SSH Banner (Before Login):${NC}"
                echo -e "${YELLOW}═══════════════════════════════════════════════════════════════${NC}"
                cat "$BANNER_DIR/ssh-banner"
                echo -e "${YELLOW}═══════════════════════════════════════════════════════════════${NC}"
                read -p "Press Enter..."
            ...
            7)
                cp "$BANNER_DIR/default" "$BANNER_DIR/ssh-banner"
                cp "$SERVER_MSG_DIR/default" "$SERVER_MSG_DIR/default.msg" 2>/dev/null
                echo "default" > "$SERVER_MSG_DIR/active"
                systemctl restart sshd 2>/dev/null
                echo -e "${GREEN}✅ Both reset to default!${NC}"
                read -p "Press Enter..."
                ;;
            8)
                clear
                echo -e "${YELLOW}Available Server Messages:${NC}"
                ls -1 "$SERVER_MSG_DIR/"*.msg 2>/dev/null
                echo ""
                read -p "$(echo -e $RED"Enter message name to DELETE: "$NC)" del_name
                if [ -f "$SERVER_MSG_DIR/$del_name" ] && [ "$del_name" != "default" ] && [ "$del_name" != "active" ]; then
                    rm -f "$SERVER_MSG_DIR/$del_name"
                    [ "$(cat "$SERVER_MSG_DIR/active")" = "$del_name" ] && echo "default" > "$SERVER_MSG_DIR/active"
                    echo -e "${GREEN}✅ Deleted!${NC}"
                else
                    echo -e "${RED}❌ Cannot delete this message!${NC}"
                fi
                read -p "Press Enter..."
                ;;
            0) return ;;
...
get_remaining_time() {
    ...
    if [ "$expire_ts" -le "$current_ts" ]; then
        echo "Expired"
    else
        local diff_sec=$((expire_ts - current_ts))
        local diff_days=$((diff_sec / 86400))
        local diff_hrs=$(((diff_sec % 86400) / 3600))
        
        if [ $diff_days -gt 0 ]; then
            local result="${diff_days}day"
            [ $diff_days -gt 1 ] && result="${diff_days}days"
            [ $diff_hrs -gt 0 ] && result="${result} + ${diff_hrs}hr"
            echo "$result"
        else
            echo "${diff_hrs}hr"
        fi
...
add_user() {
    clear
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${YELLOW}           CREATE SSH + DNS USER (FALCON ULTIMATE)             ${CYAN}║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
...
INFO
    ...
    remaining=$(get_remaining_time "$expire_date")
    
    clear
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║${YELLOW}                  USER CREATED SUCCESSFULLY                    ${GREEN}║${NC}"
    echo -e "${GREEN}╠═══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${GREEN}║${WHITE}  File Name   :${CYAN} $username${NC}"
    ...
    echo -e "${GREEN}║${WHITE}  Limit GB    :${CYAN} $bw_disp${NC}"
    echo -e "${GREEN}║${WHITE}  Connection  :${CYAN} Max $conn_limit${NC}"
    echo -e "${GREEN}║${WHITE}  Expire      :${CYAN} $expire_date ($remaining)${NC}"
    echo -e "${GREEN}╠═══════════════════════════════════════════════════════════════╣${NC}"
...
list_users() {
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${YELLOW}${BOLD}                  ACTIVE USERS + BANDWIDTH + STATUS (FALCON ULTIMATE)                       ${CYAN}║${NC}"
    echo -e "${CYAN}╠══════════════════════════════════════════════════════════════════════════════════════════════╣${NC}"
    ...
    for user in "$UD"/*; do
        ...
        remaining=$(get_remaining_time "$ex")
        ...
        if passwd -S "$u" 2>/dev/null | grep -q "L"; then
            ...
            status="${LIGHT_RED}⚠️ $remaining${NC}"
        elif [ $days_left -le 7 ]; then
            status="${YELLOW}⚠️ $remaining${NC}"
        else
...
renew_user() {
    ...
    remaining=$(get_remaining_time "$new_expire")
    echo -e "${GREEN}✅ User renewed until $new_expire ($remaining)${NC}"
}
...
details_user() {
    ...
    expire_date=$(grep "Expire:" "$UD/$username" 2>/dev/null | awk '{print $2}')
    remaining=$(get_remaining_time "$expire_date")
    conn_limit=$(grep "Conn_Limit:" "$UD/$username" 2>/dev/null | awk '{print $2}')
    conn_limit=${conn_limit:-1}
    
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${YELLOW}                   📋 USER ACCOUNT DETAILS                      ${CYAN}║${NC}"
    echo -e "${CYAN}╠═══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${CYAN}║${WHITE}  File Name   : ${GREEN}$username${NC}"
    echo -e "${CYAN}║${WHITE}  Limit GB    : ${YELLOW}${bw_limit:-Unlimited} GB${NC}"
    echo -e "${CYAN}║${WHITE}  Usage GB    : ${CYAN}${total_gb} GB${NC}"
    echo -e "${CYAN}║${WHITE}  Connection  : ${PURPLE}${current_conn}/${conn_limit} (Total connections on user ACCOUNT)${NC}"
    echo -e "${CYAN}║${WHITE}  Expire      : ${RED}${expire_date}${NC} (${GREEN}${remaining}${NC})"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
}
...
settings_menu() {
    while true; do
        ...
        echo -e "${PURPLE}║${WHITE}  [4] Banner/Message Manager  [5] Traffic Stats${NC}"
        echo -e "${PURPLE}║${WHITE}  [6] Reset All BW  [7] Toggle Auto-Ban ($ABSTATUS)${WHITE}${NC}"
        echo -e "${PURPLE}║${WHITE}  [8] Restart All   [9] Reboot VPS       [10] Uninstall${NC}"
        echo -e "${PURPLE}║${WHITE}  [11] Recompile All C Boosters  [12] Fix VPN/SSH${NC}"
        ...
        case $ch in
            ...
            10)
                ...
                [ "$c" = "YES" ] && {
                    ...
                    systemctl restart sshd 2>/dev/null
                    rm -f /etc/profile.d/elite-x-dashboard.sh /etc/profile.d/elite-x-user-details.sh
...
rm -f /etc/profile.d/elite-x-dashboard.sh /etc/profile.d/elite-x-user-details.sh 2>/dev/null
...
# Create directories
mkdir -p /etc/elite-x/{banner,users,traffic,deleted,data_usage,connections,banned,traffic_stats,bandwidth/pidtrack,server_msg,user_details}
...
[Unit]
Description=ELITE-X DNSTT Server v3.3.3
...
alias switchmsg='elite-x-banner'
EOF
...
clear
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║${YELLOW}${BOLD}  ELITE-X v3.3.3 FALCON ULTIMATE EDITION - INSTALLED!  ${GREEN}║${NC}"
echo -e "${GREEN}╠═══════════════════════════════════════════════════════════════╣${NC}"
...
echo -e "${GREEN}║${WHITE}  Version    :${CYAN} v3.3.3 Falcon Ultimate Edition${NC}"
...
echo -e "${YELLOW}Banner: editbanner | editmsg | switchmsg${NC}"
echo -e "${YELLOW}Re-login or type 'exec bash' to access the dashboard${NC}"
echo ""
echo -e "${CYAN}📋 USER DETAILS FORMAT (shown on successful login):${NC}"
echo -e "${WHITE}  File Name    : username${NC}"
echo -e "${WHITE}  Limit GB     : X GB${NC}"
echo -e "${WHITE}  Usage GB     : X.XX GB${NC}"
echo -e "${WHITE}  Connection   : current/max (Total connections on user ACCOUNT)${NC}"
echo -e "${WHITE}  Expire       : DD/MM/YYYY (Xday + Yhr)${NC}"
echo ""
