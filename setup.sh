...
#  + Bandwidth GB Limits + 3Proxy Speed + Auto-Delete
# ╚══════════════════════════════════════════════════════════════╝
set -euo pipefail
...
install_3proxy() {
    ...
    echo -e "${YELLOW}   → Cloning 3proxy from GitHub...${NC}"
    git clone https://github.com/z3apa3a/3proxy 2>/dev/null
    if [ -d /tmp/3proxy ]; then
        cd /tmp/3proxy
        echo -e "${YELLOW}   → Building 3proxy...${NC}"
        ln -s Makefile.Linux Makefile 2>/dev/null || true
        make 2>/dev/null || true
        if [ -f "bin/3proxy" ]; then
            cp bin/3proxy "$THREEPROXY_BIN"
            chmod +x "$THREEPROXY_BIN"
            echo -e "${GREEN}✅ 3Proxy compiled and installed successfully${NC}"
        elif [ -f "src/3proxy" ]; then
            cp src/3proxy "$THREEPROXY_BIN"
            chmod +x "$THREEPROXY_BIN"
            echo -e "${GREEN}✅ 3Proxy compiled and installed successfully${NC}"
        fi
    fi
    ...
    if [ ! -f "$THREEPROXY_BIN" ]; then
        ...
        cd /tmp/3proxy 2>/dev/null || { cd /tmp && git clone https://github.com/z3apa3a/3proxy && cd /tmp/3proxy; }
        make -f Makefile.Linux 2>/dev/null || true
        if [ -f "src/3proxy" ]; then
            cp src/3proxy "$THREEPROXY_BIN"
            chmod +x "$THREEPROXY_BIN"
            echo -e "${GREEN}✅ 3Proxy installed via alternative method${NC}"
        fi
    fi

    if [ ! -f "$THREEPROXY_BIN" ]; then
        echo -e "${RED}❌ 3Proxy installation failed - continuing without it${NC}"
        return 1
    fi
    ...
    while true; do
        ...
        case $ch in
            ...
            8)
                ...
                ;;
            9)
                systemctl restart dnstt-elite-x dnstt-elite-x-proxy elite-x-bandwidth elite-x-datausage elite-x-connmon 3proxy-elite sshd 2>/dev/null
