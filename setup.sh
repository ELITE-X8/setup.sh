...
DISPLAY_MODE_FILE="/etc/elite-x/display_mode"  # "banner" or "userinfo"
...
# ═══════════════════════════════════════════════════════════════
# DYNAMIC SSH BANNER/UINFO SCRIPT
# ═══════════════════════════════════════════════════════════════
create_dynamic_ssh_script() {
    echo -e "${YELLOW}📝 Creating Dynamic SSH Display Script...${NC}"
    
    cat > /usr/local/bin/elite-x-ssh-display <<'DISPLAYEOF'
#!/bin/bash
# ELITE-X Dynamic SSH Display - User Info or Banner
...
show_user_info() {
    ...
    if [ ! -f "$USER_DB/$username" ]; then
        cat "$DEFAULT_BANNER" 2>/dev/null || cat "$CUSTOM_BANNER" 2>/dev/null
        return
    fi
    ...
    local subdomain=$(cat /etc/elite-x/subdomain 2>/dev/null || "N/A")
...
if [ -z "$USERNAME" ]; then
    ...
    USERNAME=$(whoami 2>/dev/null || echo "root")
fi

if [ "$USERNAME" = "root" ]; then
    # Root gets banner
    cat "$CUSTOM_BANNER" 2>/dev/null || cat "$DEFAULT_BANNER" 2>/dev/null || echo "ELITE-X Server"
    exit 0
fi
...
settings_menu() {
    while true; do
        ...
        echo -e "${PURPLE}║${WHITE}  [9] Toggle Display Mode [Currently: $DISPLAY_STATUS]${WHITE}${NC}"
        ...
        case $ch in
            ...
            5) cp /etc/elite-x/banner/default /etc/elite-x/banner/ssh-banner; systemctl restart sshd; echo -e "${GREEN}✅ Banner Reset to Default${NC}"; read -p "Press Enter..." ;;
            6)
            ...
            9)
                ...
                if [ "$CURRENT_DISPLAY" = "userinfo" ]; then
                    echo "banner" > "$DISPLAY_MODE_FILE"
                    echo -e "${YELLOW}✅ Display Mode set to: BANNER${NC}"
                    echo -e "${GRAY}Users will see the custom/server banner on login${NC}"
                else
                    echo "userinfo" > "$DISPLAY_MODE_FILE"
                    echo -e "${GREEN}✅ Display Mode set to: USER INFO${NC}"
...
# Also remove old ForceCommand if exists
sed -i '/elite-x-ssh-display/d' /etc/ssh/sshd_config 2>/dev/null || true
...
echo "$MTU" > /etc/elite-x/mtu
echo "0" > "$AUTOBAN_FLAG"
echo "banner" > "$DISPLAY_MODE_FILE"  # Default: banner mode
...
# Create dynamic SSH display script
create_dynamic_ssh_script

# Configure SSH to use dynamic script with ForceCommand
echo -e "${YELLOW}🔧 Configuring SSH for Dynamic Display...${NC}"
# Use Match block to apply ForceCommand only for non-root users
cat >> /etc/ssh/sshd_config <<'SSHEOF'

# ELITE-X Dynamic Display
Match Group *,!root
    ForceCommand /usr/local/bin/elite-x-ssh-display "$USER"
SSHEOF

# Also add Banner as fallback for root
...
echo -e "${GREEN}║${WHITE}  Display    :${CYAN} Dynamic (Toggle: User Info / Banner)${NC}"
...
echo ""
echo -e "${YELLOW}═════════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}📋 DISPLAY MODES:${NC}"
echo -e "${WHITE}  • Banner Mode  : Shows server banner on login${NC}"
echo -e "${WHITE}  • User Info Mode : Shows user account details automatically${NC}"
echo -e "${YELLOW}  • Toggle in Settings Menu → Option [9]${NC}"
echo -e "${YELLOW}═════════════════════════════════════════════════════════════════${NC}"
echo ""
