...
#  + GB Limits + C Boosters + Auto-Delete + VPN FIX + USER INFO
# ╚══════════════════════════════════════════════════════════════╝
set -euo pipefail
...
show_minimal_banner() {
    echo -e "${CYAN}ELITE-X v3.3.2 FALCON ULTRA C - System Ready${NC}"
}
...
# ═══════════════════════════════════════════════════════════
# CREATE USER INFO DISPLAY SCRIPT
# ═══════════════════════════════════════════════════════════
create_user_info_script() {
    cat > /usr/local/bin/elite-x-user-info <<'INFOEOF'
#!/bin/bash
...
CONN_DB="/etc/elite-x/connections"
USER_FILE="$USER_DB/$USERNAME"
BW_FILE="$BW_DIR/${USERNAME}.usage"
CONN_FILE="$CONN_DB/$USERNAME"

clear

# Header
echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${YELLOW}${BOLD}                    USER CONNECTION DETAILS                    ${CYAN}║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
EXPIRE_DATE=$(grep "Expire:" "$USER_FILE" | awk '{print $2}')
CONN_LIMIT=$(grep "Conn_Limit:" "$USER_FILE" | awk '{print $2}')
CONN_LIMIT=${CONN_LIMIT:-1}
BW_LIMIT=$(grep "Bandwidth_GB:" "$USER_FILE" | awk '{print $2}')
BW_LIMIT=${BW_LIMIT:-0}

# Get bandwidth usage
if [ -f "$BW_FILE" ]; then
    BW_BYTES=$(cat "$BW_FILE" 2>/dev/null || echo 0)
else
    BW_BYTES=0
fi
BW_GB=$(echo "scale=2; $BW_BYTES / 1073741824" | bc 2>/dev/null || echo "0.00")
...
get_connection_count() {
    ...
    if [ "$count" -eq 0 ]; then
        count=$(ps aux --no-headers 2>/dev/null | grep "sshd:" | grep "@" | grep -v grep | awk '{print $1}' | grep -c "^${USERNAME}$" 2>/dev/null || echo 0)
    fi
    echo ${count:-0}
}
CURRENT_CONN=$(get_connection_count)

# Save current connections
mkdir -p "$CONN_DB"
echo "$CURRENT_CONN" > "$CONN_FILE" 2>/dev/null

# Calculate remaining time
if [ -n "$EXPIRE_DATE" ] && [ "$EXPIRE_DATE" != "N/A" ]; then
    CURRENT_TS=$(date +%s)
    EXPIRE_TS=$(date -d "$EXPIRE_DATE" +%s 2>/dev/null || echo 0)
    
    if [ "$EXPIRE_TS" -gt 0 ]; then
        REMAIN_SEC=$((EXPIRE_TS - CURRENT_TS))
        
        if [ "$REMAIN_SEC" -gt 0 ]; then
            REMAIN_DAYS=$((REMAIN_SEC / 86400))
            REMAIN_HRS=$(( (REMAIN_SEC % 86400) / 3600 ))
            REMAIN_MINS=$(( (REMAIN_SEC % 3600) / 60 ))
            EXPIRE_DISPLAY="${GREEN}${EXPIRE_DATE}${NC} (${LIGHT_GREEN}${REMAIN_DAYS}days + ${REMAIN_HRS}hrs${NC})"
        else
            EXPIRE_DISPLAY="${RED}${EXPIRE_DATE}${NC} (${LIGHT_RED}EXPIRED${NC})"
        fi
    else
        EXPIRE_DISPLAY="${YELLOW}${EXPIRE_DATE}${NC}"
    fi
else
    EXPIRE_DISPLAY="${YELLOW}N/A${NC}"
fi

# Format bandwidth
if [ "$BW_LIMIT" != "0" ] && [ -n "$BW_LIMIT" ]; then
    BW_DISPLAY="${GREEN}${BW_LIMIT} GB${NC}"
    BW_USAGE_DISPLAY="${LIGHT_GREEN}${BW_GB} GB${NC}"
    
    # Check if near limit
    BW_PERCENT=$(echo "scale=1; ($BW_GB / $BW_LIMIT) * 100" | bc 2>/dev/null || echo "0")
    if [ "$(echo "$BW_PERCENT >= 90" | bc 2>/dev/null)" = "1" ]; then
        BW_USAGE_DISPLAY="${RED}${BW_GB} GB${NC}"
    elif [ "$(echo "$BW_PERCENT >= 75" | bc 2>/dev/null)" = "1" ]; then
        BW_USAGE_DISPLAY="${YELLOW}${BW_GB} GB${NC}"
    fi
else
    BW_DISPLAY="${GRAY}Unlimited${NC}"
    BW_USAGE_DISPLAY="${LIGHT_GREEN}${BW_GB} GB${NC}"
fi

# Check if locked
if passwd -S "$USERNAME" 2>/dev/null | grep -q "L"; then
    STATUS_DISPLAY="${RED}🔒 LOCKED - Contact Admin${NC}"
elif [ "$CURRENT_CONN" -gt "$CONN_LIMIT" ]; then
    STATUS_DISPLAY="${RED}⚠️ EXCEEDED CONNECTION LIMIT!${NC}"
else
    STATUS_DISPLAY="${LIGHT_GREEN}🟢 ACTIVE${NC}"
fi

# Display info box
echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  ${WHITE}File Name  : ${GREEN}${USERNAME}${NC}"
echo -e "${CYAN}║  ${WHITE}Limit GB   : ${BW_DISPLAY}${NC}"
echo -e "${CYAN}║  ${WHITE}Usage GB   : ${BW_USAGE_DISPLAY}${NC}"
echo -e "${CYAN}║  ${WHITE}Connection : ${LIGHT_GREEN}${CURRENT_CONN}${WHITE}/${GREEN}${CONN_LIMIT}${NC}"
echo -e "${CYAN}║  ${WHITE}Expire     : ${EXPIRE_DISPLAY}${NC}"
echo -e "${CYAN}║  ${WHITE}Status     : ${STATUS_DISPLAY}${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""

# Show bandwidth progress bar if limit exists
if [ "$BW_LIMIT" != "0" ] && [ -n "$BW_LIMIT" ]; then
    BW_PERCENT=$(echo "scale=1; ($BW_GB / $BW_LIMIT) * 100" | bc 2>/dev/null || echo "0")
    BAR_LENGTH=40
    FILLED=$(echo "scale=0; $BW_PERCENT * $BAR_LENGTH / 100" | bc 2>/dev/null || echo "0")
    
    echo -ne "${WHITE}Bandwidth Usage: ${NC}["
    ...
    echo -e "${WHITE}] ${BW_PERCENT}%${NC}"
    echo ""
fi

# Connection status
echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  ${YELLOW}SERVER MESSAGE: Successfully Connected!${NC}                ${CYAN}║${NC}"
echo -e "${CYAN}║  ${WHITE}Type ${GREEN}menu${WHITE} for main menu${NC}                                 ${CYAN}║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
INFOEOF
    chmod +x /usr/local/bin/elite-x-user-info
}

# ═══════════════════════════════════════════════════════════
# SSH CONFIGURATION WITH USER INFO DISPLAY
# ═══════════════════════════════════════════════════════════
configure_ssh_for_vpn() {
    echo -e "${YELLOW}🔧 Configuring SSH for VPN + User Info Display...${NC}"
    
    # Backup original config
    cp /etc/ssh/sshd_config /etc/ssh/sshd_config.bak 2>/dev/null || true
    
    # Configure SSH for tunneling with ForceCommand for user info
...
# Force user info display on login (NOT for root)
Match Group *,!root
    ForceCommand /usr/local/bin/elite-x-user-info
SSHCONF
    ...
    echo -e "${GREEN}✅ SSH configured with User Info Display${NC}"
}
...
get_connection_count() {
    ...
    if [ -f "$CONN_DB/$username" ]; then
        count=$(cat "$CONN_DB/$username" 2>/dev/null || echo 0)
    else
        who | grep -qw "$username" 2>/dev/null && count=$(who | grep -wc "$username" 2>/dev/null)
        [ "$count" -eq 0 ] && count=$(ps aux 2>/dev/null | grep "sshd:" | grep "@" | grep -v grep | awk '{print $1}' | grep -c "^${username}$" 2>/dev/null || echo 0)
    fi
    echo ${count:-0}
}
...
show_dashboard() {
    ...
    LOCATION=$(cat /etc/elite-x/location 2>/dev/null || echo "Unknown")
...
settings_menu() {
    while true; do
        ...
        echo -e "${PURPLE}║${WHITE}  [14] Update User Info Script${NC}"
        ...
        case $ch in
            ...
            3) 
                ...
                ;;
            4) 
                if [ -f /etc/elite-x/banner/ssh-banner ]; then
                    nano /etc/elite-x/banner/ssh-banner
                else
                    echo -e "${YELLOW}Banner system simplified. Use option 14 to update user info display.${NC}"
                fi
                systemctl restart sshd 2>/dev/null
                read -p "Press Enter..."
                ;;
            5) 
                cat > /etc/elite-x/banner/default <<'DEFEOF'
ELITE-X v3.3.2 FALCON ULTRA C
DEFEOF
            ...
            6) 
                ...
                if [ -n "$iface" ]; then
                    ...
                    echo -e "RX: $(echo "scale=2; $rx/1073741824" | bc 2>/dev/null || echo "0") GB"
                    echo -e "TX: $(echo "scale=2; $tx/1073741824" | bc 2>/dev/null || echo "0") GB"
                else
                    echo -e "${RED}Cannot read traffic stats${NC}"
                fi
                read -p "Press Enter..."
                ;;
            7) 
            ...
            13)
                ...
                ;;
            14)
                echo -e "${YELLOW}Updating User Info Script...${NC}"
                create_user_info_script
                systemctl restart sshd 2>/dev/null
                echo -e "${GREEN}✅ User Info Script updated${NC}"
                read -p "Press Enter..."
                ;;
            0) return ;;
        esac
...
# ═══════════════════════════════════════════════════════════
clear
echo -e "${CYAN}ELITE-X v3.3.2 FALCON ULTRA C EDITION${NC}"
echo -e "${YELLOW}╔═══════════════════════════════════════════════════════════════╗${NC}"
...
# Create user info display script (BEFORE SSH config so ForceCommand finds it)
create_user_info_script

# Configure SSH for VPN with user info display
configure_ssh_for_vpn
...
# Setup auto-login dashboard (only for root)
cat > /etc/profile.d/elite-x-dashboard.sh <<'EOF'
#!/bin/bash
if [ "$(id -u)" = "0" ] && [ -f /usr/local/bin/elite-x ] && [ -z "$ELITE_X_SHOWN" ]; then
...
alias updateinfo='elite-x-user-info'
EOF
...
echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${YELLOW}Commands: menu | elite-x | users | adduser | setbw | boost | fixvpn | updateinfo${NC}"
echo -e "${YELLOW}Users will see their connection details automatically on login${NC}"
echo ""
...
echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║${YELLOW}  USER CONNECTION DISPLAY FORMAT:${NC}"
echo -e "${GREEN}║${WHITE}  File Name  : username${NC}"
echo -e "${GREEN}║${WHITE}  Limit GB   : X GB or Unlimited${NC}"
echo -e "${GREEN}║${WHITE}  Usage GB   : X.XX GB${NC}"
echo -e "${GREEN}║${WHITE}  Connection : X/Y${NC}"
echo -e "${GREEN}║${WHITE}  Expire     : YYYY-MM-DD (Xdays + Xhrs)${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
