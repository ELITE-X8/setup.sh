#!/bin/bash
# ╔══════════════════════════════════════════════════════════════╗
#  ELITE-X SLOWDNS SCRIPT v3.7 - PER-USER MESSAGE FIXED
# ╚══════════════════════════════════════════════════════════════╝
set -euo pipefail
...
show_banner() {
    clear
    echo -e "${PURPLE}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}║${YELLOW}${BOLD}    ELITE-X SLOWDNS v3.7 - PER-USER MESSAGE FIXED    ${PURPLE}║${NC}"
    echo -e "${PURPLE}║${GREEN}${BOLD}  GB Limits • Bandwidth • C Boosters • Auto-Delete • Per-User Info  ${PURPLE}║${NC}"
...
# ═══════════════════════════════════════════════════════════
# SSH CONFIGURATION - GENERIC BANNER + PER-USER MSG
# ═══════════════════════════════════════════════════════════
configure_ssh_for_vpn() {
    echo -e "${YELLOW}🔧 Configuring SSH for VPN + Per-User Messages...${NC}"
...
# Generic banner shown BEFORE login
Banner /etc/elite-x/server_msg/banner

# Print MOTD (message of the day)
PrintMotd yes

# Use PAM for per-user messages
UsePAM yes
SSHCONF

    echo "Include /etc/ssh/sshd_config.d/*.conf" >> /etc/ssh/sshd_config
    
    # Create generic welcome banner (shown before authentication)
    cat > /etc/elite-x/server_msg/banner <<'BANNER'
╔═══════════════════════════════════════════╗
║     ⚡ ELITE-X SLOWDNS VPN ⚡            ║
║     v3.7 Per-User Message Edition       ║
╠═══════════════════════════════════════════╣
║  🔐 Please login with your credentials  ║
║  📊 Your account details will appear    ║
...
BANNER
    ...
    echo -e "${GREEN}✅ SSH configured with Per-User Messages${NC}"
}

# ═══════════════════════════════════════════════════════════
# PER-USER MESSAGE GENERATOR (FIXED)
# ═══════════════════════════════════════════════════════════
create_per_user_message_system() {
    echo -e "${YELLOW}📝 Creating Per-User Message System (FIXED)...${NC}"
    
    mkdir -p "$SERVER_MSG_DIR"
    mkdir -p "$USER_MSG_DIR"
    
    # Create a script that generates user-specific messages
    cat > /usr/local/bin/elite-x-generate-user-msg <<'GENEOF'
#!/bin/bash
USER_DB="/etc/elite-x/users"
...
generate_user_message() {
    ...
    if [ -f "$USER_DB/$username" ]; then
        ...
        # Build the dynamic banner for THIS user
        cat > "$USER_MSG_DIR/${username}" <<BANNER
╔═══════════════════════════════════════════╗
║     ⚡ ELITE-X SLOWDNS VPN ⚡            ║
║     v3.7 Per-User Message Edition       ║
╠═══════════════════════════════════════════╣
...
if [ -n "${1:-}" ]; then
    generate_user_message "$1"
fi
GENEOF
    chmod +x /usr/local/bin/elite-x-generate-user-msg
    
    # Create PAM login script (runs AFTER user is authenticated)
    cat > /usr/local/bin/elite-x-login-display <<'PAMEOF'
#!/bin/bash
# This script runs after PAM authentication
# It displays the user's personal message

USER_MSG_DIR="/etc/elite-x/user_messages"

# PAM sets PAM_USER to the authenticated username
if [ -n "${PAM_USER:-}" ] && [ "$PAM_TYPE" = "open_session" ]; then
    # Generate fresh message for this user
    /usr/local/bin/elite-x-generate-user-msg "$PAM_USER"
    
    # Display the message
    if [ -f "$USER_MSG_DIR/$PAM_USER" ]; then
        cat "$USER_MSG_DIR/$PAM_USER"
    fi
fi

# Also update for SSH sessions
if [ -n "${USER:-}" ] && [ "$PAM_TYPE" = "open_session" ]; then
    /usr/local/bin/elite-x-generate-user-msg "$USER"
fi

exit 0
PAMEOF
    chmod +x /usr/local/bin/elite-x-login-display
    
    # Create SSH ForceCommand script (THE MAIN FIX)
    cat > /usr/local/bin/elite-x-shell-wrapper <<'WRAPEOF'
#!/bin/bash
USER_MSG_DIR="/etc/elite-x/user_messages"

# Generate fresh message for current user
/usr/local/bin/elite-x-generate-user-msg "$USER"

# Display user's personal message
if [ -f "$USER_MSG_DIR/$USER" ]; then
    cat "$USER_MSG_DIR/$USER"
    echo ""
fi

# Fallback to bash or original command
...
WRAPEOF
    chmod +x /usr/local/bin/elite-x-shell-wrapper
    
    # Create profile.d script (runs on login shell)
    cat > /etc/profile.d/elite-x-user-msg.sh <<'PROFEOF'
#!/bin/bash
USER_MSG_DIR="/etc/elite-x/user_messages"

# Only show for SSH sessions, not for root's interactive use
if [ -n "$SSH_TTY" ] && [ "$USER" != "root" ]; then
    /usr/local/bin/elite-x-generate-user-msg "$USER" 2>/dev/null
    if [ -f "$USER_MSG_DIR/$USER" ]; then
        cat "$USER_MSG_DIR/$USER"
        echo ""
    fi
...
PROFEOF
    chmod +x /etc/profile.d/elite-x-user-msg.sh
    
    # Create a systemd timer to pre-generate messages every 5 minutes
    cat > /etc/systemd/system/elite-x-msg-refresh.service <<'EOF'
[Unit]
Description=ELITE-X Per-User Message Refresh
After=network.target

[Service]
Type=oneshot
ExecStart=/bin/bash -c 'for user in /etc/elite-x/users/*; do [ -f "$user" ] && /usr/local/bin/elite-x-generate-user-msg "$(basename "$user")" 2>/dev/null; done'
EOF

    cat > /etc/systemd/system/elite-x-msg-refresh.timer <<'EOF'
[Unit]
Description=ELITE-X Per-User Message Refresh Timer (Every 5 min)
Requires=elite-x-msg-refresh.service
...
EOF
    ...
    systemctl enable elite-x-msg-refresh.timer 2>/dev/null || true
    systemctl start elite-x-msg-refresh.timer 2>/dev/null || true
    
    echo -e "${GREEN}✅ Per-User Message System ready${NC}"
}

# ═══════════════════════════════════════════════════════════
# PAM CONFIGURATION FOR PER-USER MESSAGE (FIXED)
# ═══════════════════════════════════════════════════════════
configure_pam_per_user() {
    echo -e "${YELLOW}🔧 Configuring PAM for per-user messages...${NC}"
    
    # Remove old entries
    sed -i '/elite-x/d' /etc/pam.d/sshd 2>/dev/null
    
    # Add pam_exec to run on session open (AFTER authentication)
    # This runs as the authenticated user
    if ! grep -q "pam_exec.*elite-x-login-display" /etc/pam.d/sshd; then
        sed -i '/^session.*required.*pam_selinux/i session    optional     pam_exec.so /usr/local/bin/elite-x-login-display' /etc/pam.d/sshd
    fi
    
    # Also add to common-session for all services
    if ! grep -q "pam_exec.*elite-x-login-display" /etc/pam.d/common-session 2>/dev/null; then
        echo "session    optional     pam_exec.so /usr/local/bin/elite-x-login-display" >> /etc/pam.d/common-session 2>/dev/null || true
    fi
    
    echo -e "${GREEN}✅ PAM configured for per-user messages${NC}"
}
...
# ═══════════════════════════════════════════════════════════
# USER MANAGEMENT SCRIPT (UPDATED WITH PER-USER MSG)
# ═══════════════════════════════════════════════════════════
create_user_script() {
...
add_user() {
    clear
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${YELLOW}              CREATE SSH + DNS USER (PER-USER MSG)             ${CYAN}║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
...
INFO
    ...
    # Generate per-user message for this user
    /usr/local/bin/elite-x-generate-user-msg "$username" 2>/dev/null
    ...
    echo -e "${GREEN}╠═══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${GREEN}║${YELLOW}  User will see their details on login!${NC}"
...
list_users() {
    clear
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${YELLOW}${BOLD}                    ACTIVE USERS (PER-USER MSG)              ${CYAN}║${NC}"
    echo -e "${CYAN}╠═══════════════════════════════════════════════════════════════════════════╣${NC}"
    ...
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${CYAN}║${YELLOW}  Users: ${GREEN}${TOTAL_USERS}${YELLOW} | Online: ${GREEN}${TOTAL_ONLINE}${YELLOW} | Each sees their info on login${NC}                  ${CYAN}║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════════════════════════════╝${NC}"
}
...
lock_user() { read -p "$(echo -e $GREEN"Username: "$NC)" u; [ ! -f "$UD/$u" ] && { echo -e "${RED}User not found!${NC}"; return; }; usermod -L "$u" 2>/dev/null; pkill -u "$u" 2>/dev/null || true; echo "$(date) - MANUALLY LOCKED" >> "$BD/$u"; /usr/local/bin/elite-x-generate-user-msg "$u" 2>/dev/null; echo -e "${GREEN}✅ User locked${NC}"; }
unlock_user() { read -p "$(echo -e $GREEN"Username: "$NC)" u; [ ! -f "$UD/$u" ] && { echo -e "${RED}User not found!${NC}"; return; }; usermod -U "$u" 2>/dev/null; echo "$(date) - MANUALLY UNLOCKED" >> "$BD/$u"; /usr/local/bin/elite-x-generate-user-msg "$u" 2>/dev/null; echo -e "${GREEN}✅ User unlocked${NC}"; }
delete_user() { read -p "$(echo -e $GREEN"Username: "$NC)" u; [ ! -f "$UD/$u" ] && { echo -e "${RED}User not found!${NC}"; return; }; cp "$UD/$u" "$DD/${u}_$(date +%Y%m%d_%H%M%S)" 2>/dev/null; pkill -u "$u" 2>/dev/null || true; killall -u "$u" -9 2>/dev/null || true; userdel -r "$u" 2>/dev/null; rm -f "$UD/$u" "$USAGE_DB/$u" "$CONN_DB/$u" "$BD/$u" "$BW_DIR/${u}.usage" "/etc/elite-x/user_messages/$u"; rm -rf "$PID_DIR/${u}" 2>/dev/null; echo -e "${GREEN}✅ User deleted${NC}"; }
...
details_user() {
    ...
    clear
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${YELLOW}              USER DETAILS (PER-USER MSG)            ${CYAN}║${NC}"
    echo -e "${CYAN}╠═══════════════════════════════════════════════════════════════╣${NC}"
    ...
    # Show user's personal message
    if [ -f "/etc/elite-x/user_messages/$username" ]; then
        echo -e "${CYAN}╠═══════════════════════════════════════════════════════════════╣${NC}"
        echo -e "${CYAN}║${YELLOW}  LOGIN MESSAGE PREVIEW:${NC}"
        echo -e "${CYAN}║${NC}"
        while IFS= read -r line; do
            echo -e "${CYAN}║${WHITE}  $line${NC}"
        done < "/etc/elite-x/user_messages/$username"
    fi
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
}
...
case $1 in
    ...
    renew) renew_user ;;
    setlimit) read -p "Username: " u; read -p "New limit: " l; [ -f "$UD/$u" ] && { sed -i "s/Conn_Limit: .*/Conn_Limit: $l/" "$UD/$u"; /usr/local/bin/elite-x-generate-user-msg "$u" 2>/dev/null; echo -e "${GREEN}✅ Updated${NC}"; } || echo -e "${RED}Not found${NC}" ;;
...
UD="/etc/elite-x/users"; BW_DIR="/etc/elite-x/bandwidth"; AUTOBAN_FLAG="/etc/elite-x/autoban_enabled"; USER_MSG_DIR="/etc/elite-x/user_messages"
...
show_dashboard() {
    ...
    # Check Per-User Message system
    if [ -d "$USER_MSG_DIR" ] && [ -x /usr/local/bin/elite-x-generate-user-msg ]; then
        USER_COUNT=$(ls "$USER_MSG_DIR" 2>/dev/null | wc -l)
        SMSG="${GREEN}✅ Active ($USER_COUNT users)${NC}"
    else
    ...
    echo -e "${PURPLE}╔════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}║${YELLOW}${BOLD}    ELITE-X v3.7 - PER-USER MESSAGE FIXED   ${PURPLE}║${NC}"
    echo -e "${PURPLE}╠════════════════════════════════════════════════════════════════╣${NC}"
    ...
    echo -e "${PURPLE}║${WHITE}  Per-User Msg: $SMSG${NC}"
...
settings_menu() {
    while true; do
        ...
        echo -e "${PURPLE}╔════════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${PURPLE}║${YELLOW}${BOLD}                 SETTINGS MENU (v3.7)              ${PURPLE}║${NC}"
        echo -e "${PURPLE}╠════════════════════════════════════════════════════════════════╣${NC}"
        echo -e "${PURPLE}║${WHITE}  [1] Change MTU  [2] Speed Optimize  [3] Clean Cache${NC}"
        echo -e "${PURPLE}║${WHITE}  [4] User Msgs   [5] Refresh All Msgs [6] Traffic Stats${NC}"
        ...
        echo -e "${PURPLE}║${WHITE}  [12] Recompile C  [13] Fix VPN/SSH     [14] Test User Msg${NC}"
        ...
        case $ch in
            ...
            3) 
                ...
                read -p "Press Enter..." ;;
            4) 
                ls -la /etc/elite-x/user_messages/ 2>/dev/null
                echo -e "${YELLOW}These are per-user message files${NC}"
                read -p "Press Enter..." ;;
            5) 
                echo -e "${YELLOW}Refreshing all per-user messages...${NC}"
                for user in "$UD"/*; do
                    [ -f "$user" ] && /usr/local/bin/elite-x-generate-user-msg "$(basename "$user")" 2>/dev/null
                done
                echo -e "${GREEN}✅ All messages refreshed!${NC}"
                read -p "Press Enter..." ;;
            6) 
            ...
            9) 
                ...
                echo -e "${GREEN}✅ All services restarted & messages refreshed${NC}"
                read -p "Press Enter..." ;;
            ...
            13)
                ...
                read -p "Press Enter..." ;;
            14)
                read -p "Enter username to test message: " testuser
                if [ -f "$UD/$testuser" ]; then
                    /usr/local/bin/elite-x-generate-user-msg "$testuser"
                    echo -e "${CYAN}╔═══════════════════════════════════════════╗${NC}"
                    echo -e "${CYAN}║${YELLOW}  LOGIN MESSAGE FOR: $testuser${NC}"
                    echo -e "${CYAN}╠═══════════════════════════════════════════╣${NC}"
                    cat "/etc/elite-x/user_messages/$testuser"
                    echo -e "${CYAN}╚═══════════════════════════════════════════╝${NC}"
                else
...
main_menu() {
    while true; do
        ...
        echo -e "${PURPLE}╔════════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${PURPLE}║${GREEN}${BOLD}               MAIN MENU v3.7 (PER-USER MSG)      ${PURPLE}║${NC}"
        echo -e "${PURPLE}╠════════════════════════════════════════════════════════════════╣${NC}"
        ...
        echo -e "${PURPLE}║${WHITE}  [10] Delete User  [11] Deleted List     [12] Test User Msg${NC}"
        echo -e "${PURPLE}║${WHITE}  [S] Settings      [0] Exit${NC}"
        echo -e "${PURPLE}╚════════════════════════════════════════════════════════════════╝${NC}"
        read -p "$(echo -e $GREEN"Option: "$NC)" ch
        ...
        case $ch in
            ...
            12) 
                read -p "Enter username: " tuser
                if [ -f "$UD/$tuser" ]; then
                    /usr/local/bin/elite-x-generate-user-msg "$tuser"
                    clear
                    cat "/etc/elite-x/user_messages/$tuser"
                else
                    echo -e "${RED}User not found!${NC}"
                fi
                read -p "Press Enter..." ;;
...
for s in dnstt-elite-x dnstt-elite-x-proxy elite-x-bandwidth elite-x-datausage elite-x-connmon elite-x-cleaner elite-x-traffic elite-x-netbooster elite-x-dnscache elite-x-ramcleaner elite-x-irqopt elite-x-logcleaner elite-x-banner-refresh elite-x-msg-refresh 3proxy-elite; do
    systemctl stop "$s" 2>/dev/null || true
    systemctl disable "$s" 2>/dev/null || true
done
systemctl disable elite-x-banner-refresh.timer 2>/dev/null || true
systemctl disable elite-x-msg-refresh.timer 2>/dev/null || true
systemctl stop elite-x-banner-refresh.timer 2>/dev/null || true
systemctl stop elite-x-msg-refresh.timer 2>/dev/null || true
...
rm -f /etc/profile.d/elite-x-dashboard.sh 2>/dev/null
rm -f /etc/profile.d/elite-x-user-msg.sh 2>/dev/null
sed -i '/^Banner/d' /etc/ssh/sshd_config 2>/dev/null
sed -i '/Include \/etc\/ssh\/sshd_config.d\/\*\.conf/d' /etc/ssh/sshd_config 2>/dev/null
sed -i '/elite-x/d' /etc/pam.d/sshd 2>/dev/null
sed -i '/elite-x/d' /etc/pam.d/common-session 2>/dev/null
...
# Create directories
mkdir -p /etc/elite-x/{banner,users,traffic,deleted,data_usage,connections,banned,traffic_stats,bandwidth/pidtrack,server_msg,user_messages}
...
# Create PER-USER message system (THE FIX)
create_per_user_message_system

# Configure SSH with generic banner + per-user MOTD
configure_ssh_for_vpn

# Configure PAM for per-user messages
configure_pam_per_user
...
# Generate initial per-user messages
...
#!/bin/bash
if [ -f /usr/local/bin/elite-x ] && [ -z "$ELITE_X_SHOWN" ] && [ "$USER" = "root" ]; then
...
alias refreshmsg='for u in /etc/elite-x/users/*; do [ -f "$u" ] && /usr/local/bin/elite-x-generate-user-msg "$(basename "$u")"; done && echo "✅ All messages refreshed!"'
alias testuser='read -p "Username: " tu; [ -f /etc/elite-x/users/$tu ] && cat /etc/elite-x/user_messages/$tu || echo "Not found"'
EOF
...
clear
echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║${YELLOW}${BOLD}  ELITE-X v3.7 - PER-USER MESSAGE FIXED - INSTALLED! ${GREEN}║${NC}"
echo -e "${GREEN}╠═══════════════════════════════════════════════════════════════╣${NC}"
echo -e "${GREEN}║${WHITE}  Domain     :${CYAN} $TDOMAIN${NC}"
echo -e "${GREEN}║${WHITE}  Location   :${CYAN} $SEL_LOC (MTU: $MTU)${NC}"
echo -e "${GREEN}║${WHITE}  IP         :${CYAN} $IP${NC}"
echo -e "${GREEN}║${WHITE}  Version    :${CYAN} v3.7 Per-User Message Fixed${NC}"
...
# Per-User Message check
if [ -d "/etc/elite-x/user_messages" ] && [ -x /usr/local/bin/elite-x-generate-user-msg ] && [ -f /etc/profile.d/elite-x-user-msg.sh ]; then
    echo -e "${GREEN}║  ✅ Per-User Msg   : Active (Each user sees their details!)${NC}"
else
    echo -e "${RED}║  ❌ Per-User Msg   : Inactive${NC}"
fi

echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${YELLOW}Commands: menu | elite-x | users | adduser | setbw | boost | fixvpn | refreshmsg | testuser${NC}"
echo -e "${YELLOW}Re-login or type 'exec bash' to access the dashboard${NC}"
echo ""
echo -e "${CYAN}═══ PER-USER MESSAGE FEATURE (FIXED) ═══${NC}"
echo -e "${WHITE}KILA MTUMIAJI ATAONA DETAILS ZAKE ANAPOCONNECT:${NC}"
echo -e "${WHITE}  👤 USERNAME • 📅 EXPIRE • ⏳ REMAINING (Xday + Yhr)${NC}"
echo -e "${WHITE}  📊 LIMIT GB • 💾 USAGE GB • 🔗 CONNECTIONS${NC}"
echo -e "${WHITE}  🔄 Messages auto-refresh every 5 minutes${NC}"
echo ""
echo -e "${CYAN}MECHANISM (JINSI INAVYOFANYA KAZI):${NC}"
echo -e "${WHITE}  1. Generic banner kabla ya login (kwa wote)${NC}"
echo -e "${WHITE}  2. /etc/profile.d/elite-x-user-msg.sh inarun kwa kila user${NC}"
echo -e "${WHITE}  3. Inagenerate ujumbe binafsi kwenye /etc/elite-x/user_messages/{username}${NC}"
echo -e "${WHITE}  4. Inaonyesha ujumbe huo mara baada ya login${NC}"
echo ""
